...
-- 1c…01 dono que some, 1c…02 administrador, 1c…03 membro comum,
...
select is(
  (select count(*)::integer from public.account_recovery_keys where user_id = '1c000000-0000-0000-0000-000000000001'),
  0,
  'a chave de recuperacao e destruida junto'
);
...
select is(
  (select status from public.verify_recovery_key('recovery-owner@janja.local', 'ccccccccccccccccccccccccccccccccccccccccccc')),
  'invalid',
  'uma lapide nao volta pela chave que tinha'
);
reset role;
