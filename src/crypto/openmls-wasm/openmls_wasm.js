...
export class Group {
     ...
     * @param {Provider} provider
     * @param {Identity} sender
     * @param {string} removed_identity
     * @returns {RemoveMessages}
     */
    propose_and_commit_remove(provider, sender, removed_identity) {
        _assertClass(provider, Provider);
        _assertClass(sender, Identity);
        const ptr0 = passStringToWasm0(removed_identity, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
...
export class Provider {
     ...
     */
    export_state() {
        const ret = wasm.provider_export_state(this.__wbg_ptr);
        if (ret[3]) {
            throw takeFromExternrefTable0(ret[2]);
        }
        var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
     ...
     * @returns {Provider}
     */
    static from_state(bytes) {
        const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.provider_from_state(ptr0, len0);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
...
export class RemoveMessages {
     ...
     */
    get proposal() {
        const ret = wasm.removemessages_proposal(this.__wbg_ptr);
        return ret;
    }
}
...
const RemoveMessagesFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_removemessages_free(ptr, 1));
