...
drop function if exists public.update_role(
  uuid, text, text, bigint, boolean, boolean
);
...
begin
  ...
  if not found or not public.can_manage_role(p_role_id) then
  ...
  perform public.write_audit(
    ...
    jsonb_build_object(
      ...
      'after', jsonb_build_object(
        ...
        'icon', v_after_emoji,
        'hoist', case when v_role.is_default then false else p_hoist end,
        'mentionable', p_mentionable
      )
    )
  );
